/* TinyUSB configuration for the WCH CH32H41x.
 *
 * Device only, USBFS on OTG_FS, no OS.
 *
 * The CH32H41x is a separate port from the CH32V series next door: this part
 * has the newer USBFS IP, and its dcd is dcd_ch32_usbfs.c selected by
 * OPT_MCU_CH32H417. The bundled TinyUSB in this fork is the version that knows
 * about it -- upstream Adafruit ships 0.20, which does not.
 *
 * The core supplies the three TinyUSB_Port_* functions and the USBFS interrupt
 * (see cores/ch32h4/ch32h4_usb_adafruit.cpp and ch32h4_usb.c), so there is no
 * Adafruit_TinyUSB_ch32h4.cpp here.
 */
#pragma once
 ...
 *
 * THIS FILE HAS A TWIN, cores/ch32h4/tusb_config.h in the core repository.
